        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    @if (isset($data))
                        <form class="w-full" action="{{ action('MonitoringMangroveController@update', $data->idmonev) }}"
                            method="post" enctype="multipart/form-data">
                            @method('PUT')
                        @else
                            <form class="w-full" action="{{ action('MonitoringMangroveController@store') }}"
                                method="post" enctype="multipart/form-data">
                    @endif
                    @csrf
                    <div class="flex flex-wrap -mx-3 mb-6">
                        ...
                        <div class="flex flex-wrap w-full">
                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                <label class="flex block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2 "
                                    for="grid-password">
                                    Jumlah Mati &nbsp; <div class="text-red-400"> *</div>
                                </label>
                                <input
                                    class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 @error('jmlmati') border-red-500 @enderror"
                                    type="number" placeholder="" name="jmlmati"
                                    @if ($errors->any()) value="{{ old('jmlmati') }}"
                                    @else
                                    @isset($data)
                                    value="{{ $data->jml_mati }}"
                                    @endisset @endif>
                            ...
                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                <label class="flex block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2 "
                                    for="grid-password">
                                    Jumlah Hidup &nbsp; <div class="text-red-400"> *</div>
                                </label>
                                <input
                                    class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 @error('jmlhidup') border-red-500 @enderror"
                                    type="number" placeholder="" name="jmlhidup"
                                    @if ($errors->any()) value="{{ old('jmlhidup') }}"
                                    @else
                                    @isset($data)
                                    value="{{ $data->jml_hidup }}"
                                    @endisset @endif>
                                @error('jmlhidup')
                                    <strong class="font-bold text-red-500">{{ $message }}!</strong>
                                @enderror
                        ...
                        <div class="w-full px-3">
                            ...
                            <div class="w-full px- mb-3">
                                ...
                                @isset($data)
                                    <img id="oldpic" src="{{ url('/image/penanamanmonev/' . $data->foto) }}"
